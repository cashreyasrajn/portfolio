export function TrustBar() {
  ...
  return (
    <section className="border-t border-primary/8 py-20 md:py-28">
      <div className="mx-auto max-w-[1280px] px-6 md:px-12">
        <p className="mb-10 text-center font-body text-[11px] font-bold uppercase tracking-[0.2em] text-on-surface-variant/40">
        ...
        <div className="flex flex-wrap items-center justify-center gap-6 opacity-30 grayscale transition-all duration-500 hover:opacity-60 hover:grayscale-0">
          {logos.map((logo) => (
            <div
              key={logo.name}
              className="flex h-16 w-32 items-center justify-center rounded-md border border-primary/10 bg-white"
              title={logo.name}
            >
              <span className="font-headline text-[22px] font-bold text-primary/30">
                {logo.letter}
              </span>
            </div>
          ))}
